...
% Last Modified by GUIDE v2.5 09-May-2013 17:46:49
...
start(handles.kam);
pause(0.5);
handles.loadImage = flipdim(flipdim(getsnapshot(handles.kam),1),2);
stop(handles.kam);

set(handles.edtOpenFile, 'String', 'Camera');
set(handles.btnReadValues, 'Enable', 'on'); 
axes(handles.imLoad);
imshow(handles.loadImage);
axis off;
...
set(handles.btnSnapshot, 'Enable', 'on');
[handles.resImage handles.resValues err] = MainFunction(handles.loadImage);
if (err == 0)
    ...
    imshow(handles.loadImage);
    axis off;
    set(handles.lbResistorsValues,'string','');
    handles.resImage = handles.loadImage;
    handles.resValues = '';
end
...
if ~isequal(FileName,0)  
    imwrite(handles.loadImage, fullfile(PathName, FileName));
end
