...
for j = 1:1%length(areaIndex)    % all areas
    iAr = areaIndex(j);
...
%     input('asa');
    ...
    rotActiveTempl2 = rotActiveTempl(yOff1+1:end-yOff2,xOff1+1:end-xOff2);
    
%     edges = edge(rotActiveTempl2,'sobel');
%     [yy xx] = find(edges == 1);
%     plot(round(box(iAr,1))+xx-bx-xOff1/2, round(box(iAr,2))+yy-by-yOff1/2, 'r.');
    
    edges = edge(rotActiveTempl,'sobel');
    [yy xx] = find(edges == 1);
    xx = round(box(iAr,1))+xx-bx-xOff1/2-xOff1;
    yy = round(box(iAr,2))+yy-by-yOff1/2-yOff2;
    t=[xx,yy];
    plot(t(:,1), t(:,2), 'g.');
end
disp('Total time: ');
